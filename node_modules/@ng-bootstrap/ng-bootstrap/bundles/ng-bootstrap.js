        ...
        if (this.autoClose) {
            this._ngZone.runOutsideAngular(function () {
                var escapes$ = rxjs_1.fromEvent(_this._document, 'keyup')
                    .pipe(operators_1.takeUntil(_this.openChange.pipe(operators_1.filter(function (opened) { return !opened; }))), operators_1.filter(function (event) { return event.which === key_1.Key.Escape; }));
                var clicks$ = rxjs_1.fromEvent(_this._document, 'click')
                    .pipe(operators_1.takeUntil(_this.openChange.pipe(operators_1.filter(function (opened) { return !opened; }))), operators_1.filter(function (event) { return _this._shouldCloseFromClick(event); }));
                rxjs_1.race([escapes$, clicks$]).subscribe(function () { return _this._ngZone.run(function () { return _this.close(); }); });
            });
        }
    };
    /**
