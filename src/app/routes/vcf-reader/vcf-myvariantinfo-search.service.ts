/**
 * This is the same file as src/app/routes/entry-and-visualization/genomic-data-providers/myvariantinfo-search.service.
 *    There are some changes to reflect that this file is meant for getting information useful in VCF translations,
 *      while the other file is just for a more basic querying. The difference is that this file asks the API for 1000 entries (the max that it allows),
 *      and that we specifically ask for the vcf key, to get the position and ref/alt alleles.
 * 
 * MyVariant.info compiles variant database information from across the web and provides in an easy-to-query
 * online API.
 */
import { Observable } from "rxjs/Observable";
import { Classification, Gene, GeneReference, Variant, VariantReference } from "../entry-and-visualization/genomic-data";
import { VCFVariantReference } from "./vcf-objects"
...
import { IVariantDatabase } from "../entry-and-visualization/variant-selector/variant-selector.service";
...
@Injectable()
export class VCFMyVariantInfoSearchService implements IVariantDatabase {
  ...
  lastSuggestionSet: Observable<VCFVariantReference[]> = Observable.of<VCFVariantReference[]>([]);
   ...
   */
  public searchByString = (searchTerm: string): Observable<VCFVariantReference[]> => {
    // Get new keywords.
    ...
    for (const newKeyword of newKeywords) {
      ...
      const quickQuerySuffix = "&fields=_id&size=1000";
    ...
    const getVariantArrayObservable = (): Observable<VCFVariantReference[]> => {
      ...
      // Add suffix.
      finalQuery = finalQuery + "&fields=" + this.referenceFieldsIncludeString + "vcf&size=1000";
      ...
      return this.http.get(finalQuery)
        .map(result => {
          ...
          const variantResults: VCFVariantReference[] = [];
          var entrezId = 0;
          // For every result.
          for (const hit of result["hits"]) {
            ...
            var ref = hit.vcf.ref;
            var alt = hit.vcf.alt;
            var pos = Number(hit.vcf.position);
            // Gene construction.
            var variantGene: GeneReference = null;
            if (entrezId == 0) {
              variantGene = new GeneReference(ensureValidString(this.jsonNavigator.mergePathsData(hit, MY_VARIANT_LOCATIONS.GeneHUGO, false)[0]), Number(this.jsonNavigator.mergePathsData(hit, MY_VARIANT_LOCATIONS.EntrezID, false)[0]));
              entrezId = variantGene.entrezID;
            }
            else {
              variantGene = new GeneReference(ensureValidString(this.jsonNavigator.mergePathsData(hit, MY_VARIANT_LOCATIONS.GeneHUGO, false)[0]), entrezId);
            }
            // Variant construction
            variantResults.push(new VCFVariantReference(variantGene, ensureValidString(this.jsonNavigator.mergePathsData(hit, MY_VARIANT_LOCATIONS.VariantName, false)[0]), hit._id, ref, alt, pos));
          }
